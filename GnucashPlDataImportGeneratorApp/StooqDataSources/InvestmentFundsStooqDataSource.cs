...
{
    public class InvestmentFundsStooqDataSource : IPriceDataSourceForCommodityNamespace
    {
        StooqWebsiteClient _client;
        public InvestmentFundsStooqDataSource(StooqWebsiteClient client)
        {
            _client = client;
        }
        ...
        public async Task<Dictionary<string, List<CommodityPriceData>>> GetPricesData(IEnumerable<LastPriceDateDto> lastPricesData, string baseCurrency)
        {
            ...
            {
                var data = await _client.GetData(lastPriceData.Mnemonic, lastPriceData.Date, DateTime.Today.AddDays(-1));
