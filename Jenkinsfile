	...
	stages {
		stage('PR') {
			...
			stages {
				...
				stage('Test PR') {
					steps {
						bat 'dotnet test -v normal -l:trx\\;LogFileName=TestOutput.trx --filter /p:CollectCoverage=true /p:CoverletOutput=CodeCoverageResults/ /p:CoverletOutputFormat=cobertura || true'
